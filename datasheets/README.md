...
本目录为网上检索后下载的厂商/镜像 PDF。HW504 无独立官方 datasheet，等价件为 KY-023 / PS2 双轴摇杆。
...
| HW504 摇杆 | 行空板 K10，ADC + SW | `HW504_PS2-Joystick_HandsOnTec.pdf` | HandsOnTec PS2 Joystick（HW504 等价） |
| HW504 等价手册 | 同上 | `KY-023_JoyIT_Manual.pdf` | Joy-IT KY-023 手册 |
| CD4066BE | ~~玩具侧，并联原按键~~ **已否决，留档** | `CD4066B_TI.pdf` | TI CD4066B。触点实测 3.7V 高于 3.3V，改用 AO3400A 低边 N-MOS（datasheet 尚未入库） |
...
- **HW504**：双 10 kΩ 电位器 + 按下开关；3.3–5 V，接 ESP32 时用 3.3 V。引脚 GND / +5V / VRx / VRy / SW。摇一次换一档，不改模式。
- **AO3400A**（低边 N-MOS，SOT-23：1=G / 2=S / 3=D）：Vgs(th) 典型 0.9 V，3.3 V 栅压完全饱和；无需供电轨，源极即地。栅极经 1 kΩ 接 GPIO7 并 **47 kΩ 下拉**（浮空栅极实测 D-S 约 200 kΩ 半导通，会误触发）；漏极串 **10 kΩ** 后接原按键触点 A。那颗 10 kΩ 把支路电流封在 0.37 mA，是「不驱动电机」的硬件级保证，不可省。触点两端不加电容。
