...
## 0.3.0-demo — 2026-08-29（九档与正常关机语义）

实机联调确认原板长按开机后落在第 1 档，目标 N 可由 `N-1` 次短按到达；产品决定
开放原板全部九档，因此 `LEVEL_MAX` 从 8 调整为 9。软件只发送目标档位，不发送
“第 9 档之后”的额外短按；降低目标时仍先关机、开机回到第 1 档再逐档到目标。

`set_level.level=0` 明确为用户主动的正常关机，不设置安全闩锁。红色 `stop` 仍是
安全停机，恢复仍然只能在玩具上长按 BOOT 两秒。自动控制不能发送 level 0。
