...
| 情景模式人设列表 + 自定义/新增 | 已有人设点选后经通话接入动画进入实时通话；预设为顾深 / 阿北（男友）与阿月（女友）。自定义可手写或问卷；问卷优先。自由文本捏人仍是演示，审核流后续 | 本轮 `feat/scenario-boyfriend-personas` |
...
| 按住说话 / TTS / Waifu | 台词走 `POST /v1/speech/speak`。Chat 每轮带 `tts_style`（不念出）。默认 MiniMax `speech-02-turbo` + `junlang_nanyou`，设置页可切小米 MiMo（风格走 user 消息、台词走 assistant）。失败再降 CosyVoice。自定义声线本轮只本地记下，不打克隆接口。Waifu 未实现 | 本轮 `feat/scenario-boyfriend-personas` |
...
| 身心记录 | 独立底栏「记录」 | 最近一次、近期图谱、近期睡眠摘要、非医疗身心参考、和 AI 聊聊自己 | 只读；不能恢复设备运行，也不能产生设备动作 |
...
记录（长页下滑）
  ├─ 这一次：最近一条时长 / 模式 / 最高档 / 温感压力 / 低频小图
  ├─ 近期图谱：最近 5 次有效记录
  ├─ 近期睡眠：夜间心率休息条 + 心绪对照（本机，不发给模型）
  ├─ 身心参考：质量与趋势方向，不是健康检测
...
选择页只保留 `自由档位` 和 `失控模式`，不出现人设、情景或聊天入口。自由档位使用页：八档 Slider，仅松手发送；档位 0 为停止；固定大停止按钮始终可见。失控模式：时间配置与健康提示 → 二次确认 → 独立使用页；AI 不能延长计时或恢复停止状态。当前仓库控制页已去掉情景按钮，停止、Slider 与失控同页；失控独立确认页仍属后续 PR。

### 8.4 身体笔记与“了解自己”

身心记录是独立底栏模块，不再放在亲密时刻里。单次记录固定包含：会话概览、档位时间线、压力节律、表面温感、心率图和覆盖率、可能高响应区间、结束原因、数据质量、用户主观反馈和可编辑总结。
...
当前仓库已改为单一「和 AI 聊聊自己」入口；旧 `#/intimacy/notes*` 重定向到 `#/records*`。整段对话默认不持久化，只有用户点击“保存这条发现”才写入可编辑、可删除笔记。不能给出医学结论、性功能诊断、确定的愉悦判断、偏好定论或任何设备动作。文案走「使用数据 + 身心参考」，不写检测高潮、诊断或核心体温。
...
| B1 | 四栏导航、人设列表+表单、自我控制去情景钮、身心记录单入口 | 本轮已交付导航/列表/表单/记录长页；微信聊天与失控确认页仍后续 |
...
Draft PR `feat/intimacy-records-ia` 独立开，不自动合并，也不继续往 #6 里塞。微信式情景聊天与传感器接入拆后续短 PR。
...
- [x] 四栏底栏、情景人设列表/新建表单、自我控制去情景钮、身心记录长页与聊聊入口。
- [x] 身体笔记会话存储与自我探索对话、删除的联调切片（现挂在「记录」）。
