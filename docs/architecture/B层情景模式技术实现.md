...
“我的节奏”先作为**选择页**存在，不把选择入口和运行中的控制组件放在同一层。选择页只负责选择人设或进入失控模式，不显示可调档 Slider，也不放运行中的设备控制；用户完成选择和确认后，才进入独立的**使用页**。

选择页的三个入口是：两个用于人设，一个用于失控模式。三者使用大尺寸按钮/卡片，采用 2+1 布局，保证单手握持时主要入口位于拇指热区。
...
| 按钮 | 推荐副标题 | 点击后行为 |
| --- | --- | --- |
| `常用人设` | `与最近的人设继续` | 打开独立的人设选择页/底部抽屉；选择后进入人设预览和开始确认，不直接发设备调档 |
| `新建人设` | `创建新的陪伴` | 打开独立的人设编辑页；保存后进入预览和开始确认，不直接进入运行中的控制页 |
| `定时失控` | `进入定时模式` | 打开独立的失控配置页；选择时长、阅读规则并确认后，才进入失控使用页等待设备确认 |

交互细节：

- 没有已保存人设时，`常用人设` 显示空状态，点击后进入独立的 `新建人设` 流程，不显示假数据。
- 已保存多个人设时，`常用人设` 显示最近使用的人设摘要，点击进入独立的完整列表；列表支持置顶、归档和删除，但不提供设备命令。
- `新建人设` 不允许直接填写可执行 Prompt，只能基于审核模板配置名称、头像、声音、语气、主动性、关系设定和边界。
- `定时失控` 使用醒目的独立色彩，但不采用人设头像或 Agent 气泡，避免用户误以为是角色在控制设备。
- 选择页不显示 Slider、当前档位和运行中倒计时；这些内容只属于使用页。
- 进入人设预览、编辑人设或失控时长确认时，不自动改变当前档位；只有进入使用页并完成开始确认后，才执行允许的控制命令。
- 人设使用页展示对话、趋势和停止；失控使用页展示倒计时、档位、趋势和停止。两个使用页可以共享控制组件，但必须是选择完成后的独立页面。
- 运行中的失控模式把倒计时固定放在使用页控制区上方，`停止` 始终可点；删除入口放在结束记录卡片或“数据与隐私”中，不与停止按钮重叠。
...
我的节奏
  ├─ 选择页
  │   ├─ 常用人设 -> 人设列表 -> 人设预览 -> 开始确认 -> 人设使用页
  │   ├─ 新建人设 -> 人设编辑页 -> 预览确认 -> 保存 Persona -> 人设使用页
  │   └─ 定时失控 -> 失控配置页 -> 时长/规则确认 -> 失控使用页 -> 设备确认 -> 本地倒计时 -> stop
