...
这些不是风格问题。PR 触碰下列任何一条，必须在描述里写明为什么安全：

- **不做医疗器械。** `insert_state` 对外只说「是否在使用中」，屏上、App 上、文档里都不出现解剖学或诊断表述。
- **不做高潮检测。** FSR 只用于贴合与 1–2 Hz 节律。
- **CD4066 不驱动电机。** 只并联原按键；电机始终由原产品控制板驱动。
- **档位上限 8，原板第 9 档不可达。** 这就是 demo 上的 90% 封顶，不要为了「再大一点」改 `LEVEL_MAX`。
- **停机不能远程解除。** `resume` 只由 K10 板载 A+B 长按两秒产生。App 与云端写 `resume` 必须被拒绝。
- **原始 12 Hz 传感器流与麦克风音频不上云。** 云端只收离散枚举摘要。
- **DHT11 不是安全通道。** 禁止用 `env_temp` 做过温熔断。
- **安全功能永不收费。**
