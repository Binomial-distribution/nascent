...
void BlePeripheral::begin(DownlinkHandler handler) {
  handler_ = handler;
  g_self = this;
...
void BlePeripheral::notify(const char *json, size_t len) {
...
void BlePeripheral::handleConnect(uint32_t now_ms) {
  ...
  publishInfo();
  Serial.println("[ble] App 已连接，已签发新会话令牌");
}
...
void BlePeripheral::handleWrite(const char *data, size_t len, uint32_t now_ms) {
  ...
  // resume 不是远端能发的指令。它只由 K10 上的物理双键确认产生，
  // 让 App 能解除安全词闩锁，等于把最后一道保险交回给软件。
  ...
  // 规则 3：档位越界直接丢弃，**不钳位**。
  // 钳位只用于摇杆这种可信来源；远端发来的越界值说明对端有 bug 或恶意，
  // 悄悄改成 8 档执行比拒绝危险得多。
  ...
  if (cmd == NL_CMD_SET_JOYSTICK) {
    if (doc["enabled"] | false) out.flags |= 0x01;
    if (doc["hold_ramp"] | false) out.flags |= 0x02;
  }
