// AO3400A 低边 N-MOS：并联原产品的轻触按键。
//
// 边界必须记清楚：
//   - 它只"按按钮"，**不驱动电机**。电机始终由原产品控制板驱动。
//   - 漏极串了 10kΩ，这条支路的电流被封在 0.37mA（3.7V / 10kΩ）。
//     那颗电阻是安全论证的一部分，不是可选件：AO3400A 本体能过 5.7A，
//     拆掉它就等于把"物理上无法驱动电机"这条保证降级成一句口头纪律。
//     原板按键节点是兆欧级高阻输入（实测 200kΩ 即可触发），10kΩ 导通时
//     只在节点上留约 80mV，对功能毫无影响。
//   - 栅极必须有 47kΩ 下拉。浮空栅极实测半导通约 200kΩ，足以误触发开机。
//   - 原实体按钮不拆，手按和软件按并存。
//
