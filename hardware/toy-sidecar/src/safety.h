...
//
// 其它模块（ESP-NOW 收到的指令、入体推断、链路状态）只能向它提供输入，
// 谁都不能绕过它直接调 Cd4066::requestLevel。main.cpp 里只有一处出口。
//
// 优先级从高到低：
//   1. 闩锁停机（安全词 / 急停）—— 只能被显式复位清除，断电也算
//   2. 链路丢失 —— 超过 NL_LINK_TIMEOUT_MS 收不到 K10，立即归零
...
class SafetyGovernor {
 public:
  ...
  // 闩锁停机后唯一的解除入口：必须由 K10 收到用户显式确认才调用。
  // 自动恢复是绝对禁止的——用户喊停之后设备自己动起来是最坏的失败模式。
