...
协议 `NL_LEVEL_MAX = 9`，与原板九档一致。`requestLevel(9)` 最多只会按到第 9 档，
不会再发一次短按；降低目标时先关机、开机回到第 1 档再逐档到目标。
...
- 原板第 9 档之后再短按是回第 1 档还是关机，还没测。产品目标控制不会发送这次
  额外短按；只有联调页的原始 `press_key(tap)` 能人工触发，操作前必须确认实机状态
