...
async def add_memory(request: MemoryWriteRequest) -> MemoryItem:
    return await memory_provider.add(user_id=request.user_id, persona_id=request.persona_id, text=request.text)


@router.get("/memory", response_model=list[MemoryItem])
async def search_memory(user_id: str, persona_id: str, query: str = "", limit: int = 5) -> list[MemoryItem]:
    return await memory_provider.search(user_id=user_id, persona_id=persona_id, query=query, limit=min(limit, 20))


@router.delete("/memory/{memory_id}")
async def delete_memory(memory_id: str, user_id: str, persona_id: str) -> dict[str, bool]:
    return {"deleted": await memory_provider.delete(user_id=user_id, persona_id=persona_id, memory_id=memory_id)}


@router.delete("/memory")
async def delete_memory_scope(user_id: str, persona_id: str | None = None) -> dict[str, int]:
    return {"deleted_count": await memory_provider.delete_scope(user_id=user_id, persona_id=persona_id)}
...
async def confirm_template(user_id: str, template_payload: PersonaTemplate) -> PersonaTemplate:
    if template_payload.source != "custom" or template_payload.status != "draft":
        raise HTTPException(status_code=400, detail="only custom draft templates can be confirmed")
...
async def observe_preference(observation: PreferenceObservation) -> PreferenceRecordResponse:
...
async def list_preferences(user_id: str, persona_id: str, template_id: str) -> list[PreferenceSnapshot]:
    return await preference_store.list_scope(user_id=user_id, persona_id=persona_id, template_id=template_id)


@router.delete("/preferences")
async def delete_preferences(user_id: str, persona_id: str, template_id: str | None = None) -> dict[str, int]:
    return {"deleted_count": await preference_store.delete_scope(user_id=user_id, persona_id=persona_id, template_id=template_id)}
